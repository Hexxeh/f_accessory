...
static inline struct acc_dev *func_to_dev(struct usb_function *f) {
    return container_of(f,
    struct acc_dev, function);
}
...
static ssize_t acc_read(struct file *fp, char __user
...
static ssize_t acc_write(struct file *fp, const char __user
...
*/
req->
zero = ((xfer % dev->ep_in->maxpacket) == 0);
}
if (
copy_from_user(req
->buf, buf, xfer)) {
...
kill_all_hid_devices(struct acc_dev *dev) {
    ...
    {
        hid = list_entry(entry,
        struct acc_hid_dev, list);
        list_del(&hid->list);
...
static struct acc_instance *to_fi_acc(struct usb_function_instance *fi) {
    return container_of(fi,
    struct acc_instance, func_inst);
}
